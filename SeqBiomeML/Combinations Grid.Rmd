...
description_file <- "C:/Users/jacko/Documents/SeqBiomeML/SeqBiomeML/DESCRIPTION"
...
```{r}
comp_data <- S_species[rownames(S_species) %in% S_meta$SampleID, ]

S_species1_288_run <- pipeline_iteration_nested(new_comb[c(1, 5, 9), ], dataset = comp_data, metadata = S_meta, outcome_colname = "ParticipantType", group_colname = "SubjectID", seed = 0)

S_species1_288_res = do.call(rbind, lapply(S_species1_288_run, function(x) {
  perf = x[[1]][[6]]
...
save(S_species1_288_res, S_species1_288_run, file = "s_species1_288.RData")
```
...
models = rownames(S_species1_288_res)
test = cbind(models, S_species1_288_res)
test = test[order(test$AUC, decreasing = T), ]

top_models_names = head(test, 10)
top_models = lapply(top_models_names$models, function(x) S_species1_288_run[[x]])
names(top_models) = top_models_names$models

testdata = sapply(top_models, function(x) colnames(x[[1]][[2]]))
ploty = as.data.frame(sort(table(testdata), decreasing = T))

get_feature_frequency <- function(freq_tbl, N) {
  across_100 = freq_tbl[freq_tbl$Freq == N, ]
  across_75 = freq_tbl[freq_tbl$Freq == round(N*0.75) & freq_tbl$Freq < N, ]
  ...
  return(list("100%" = across_100, "75-99%" = across_75, "50-74%" = across_50, "25-75%" = across_25, "<25%" = across_0))
}

freq_out <- get_feature_frequency(ploty, 3)
freq_data <- data.frame(Object = names(freq_out), Count = sapply(freq_out, nrow))
freq_data$Object = factor(freq_data$Object, levels = unique(freq_data$Object))

ggplot(freq_data, aes(x = Object, y = Count, fill = Object)) +  # Different colors for each bar
  ...
  scale_y_continuous(breaks = seq(0, max(freq_data$Count) + 10, by = 10), expand = expansion(mult = c(0, 0.2)))  # Control y-axis breaks

feat_imp = lapply(top_models, function(x) {
...
sig_features = lapply(feat_imp, function(x) x[[1]][x[[1]][["pvalue"]] <= 0.05, ])
sig_features = do.call(rbind, lapply(feat_imp, function(x) x[[1]][x[[1]][["pvalue"]] <= 0.05, ]))

all_sig_feat <- unique(sig_features$feat)
sig_data <- ploty[ploty$testdata %in% all_sig_feat, ]

ggplot(sig_data, aes(x = testdata, y = Freq)) +
  geom_bar(stat = "identity") +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)))  # Control y-axis breaks

feat_imp[[2]]$Permutation[order(feat_imp[[2]]$Permutation$perf_metric, decreasing = T), ]

best_perf <- sig_features[order(sig_features$perf_metric_diff), ]
top_10_perf <- best_perf[1:10, ]
```
