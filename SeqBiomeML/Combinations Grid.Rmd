...
```{r}
model_res = S_species1_288_res
model_run = S_species1_288_run

# After getting the results
# orders the results by decreasing AUC value
models = rownames(model_res)
new_mod = cbind(models, model_res)
ord_mod = ord_mod[order(new_mod$AUC, decreasing = T), ]
...
ggplot(group_freq, aes(x = Object, y = Count, fill = Object)) +  # Different colors for each bar
...
# plots the significant features and either the feature frequency of significance or occurrence across all models
# Either sig_data or count_sig
ggplot(sig_data, aes(x = testdata, y = Freq)) +
  ...
  scale_y_continuous(limits = c(0, nrow(model_res)), expand = expansion(mult = c(0, 0.2)))  # Control y-axis breaks

# returns the best models sorted by pvalue
best_perf <- sig_features[order(sig_features$pvalue), ] # pvalue can be replaced with a different metric
top_10_perf <-head(best_perf, 10)
```
